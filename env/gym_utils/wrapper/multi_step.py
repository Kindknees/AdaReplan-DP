...
class MultiStep(gym.Wrapper):
    ...
    def __init__(
        ...
        verbose=False,
        random_n_action_steps=False,
        min_n_action_steps=1,
        random_seed_offset=23456,
        **kwargs,
    ):
        super().__init__(env)
        ...
        # Random-truncation training scheme: when set, each step()'s action
        # array (length n_action_steps) is truncated to a uniformly random t
        # in [min_n_action_steps, n_action_steps]. Models "per-chunk random
        # force-replan" during training so the diffusion sees mid-chunk
        # interrupt states AND chunk internal actions get direct reward
        # signal (positions 0..t-1 all execute, only positions t..H-1 wasted).
        # No effect when n_action_steps == 1.
        self.random_n_action_steps = bool(random_n_action_steps)
        self.min_n_action_steps = int(min_n_action_steps)
        assert 1 <= self.min_n_action_steps <= max(1, n_action_steps), (
            f"min_n_action_steps must be in [1, n_action_steps]; got "
            f"min={self.min_n_action_steps} n={n_action_steps}"
        )
        self._random_seed_offset = int(random_seed_offset)
        self._random_rng = np.random.default_rng()

    def seed(self, seed=None):
        # Decorrelate our truncation RNG from env / policy / other-wrapper RNGs
        # via offset, so the random-t draws don't shift the env's reset stream.
        if seed is not None:
            self._random_rng = np.random.default_rng(seed + self._random_seed_offset)
    ...
    def step(self, action):
        ...
        With random_n_action_steps=True, the action array is truncated to a
        uniformly random t in [min_n_action_steps, len(action)] BEFORE the
        sub-step loop. Positions t..len(action)-1 are silently discarded.
        Reward / done / info are aggregated over the t sub-steps actually
        executed. The caller (training agent) does not need to know about the
        truncation — its "1 outer step = 1 chunk sample = 1 PPO transition"
        bookkeeping is unaffected; only the env-step count per transition
        becomes random.
        """
        if action.ndim == 1:  # in case action_steps = 1
            action = action[None]
        # Random truncation. We do this BEFORE the loop so the rest of the
        # method is identical to the deterministic path.
        if self.random_n_action_steps and action.shape[0] > 1:
            t = int(
                self._random_rng.integers(
                    self.min_n_action_steps, action.shape[0] + 1
                )
            )
            action = action[:t]
        truncated = False
        ...
        # Expose how many sub-steps were actually executed this outer step.
        # Useful for diagnostics when random_n_action_steps is enabled.
        info["n_action_steps_executed"] = int(act_step + 1)
