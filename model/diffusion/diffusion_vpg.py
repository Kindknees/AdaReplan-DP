...
class VPGDiffusion(DiffusionModel):
    ...
    def forward_d3p(self, cond, adaptor, deterministic=False, return_chain=True,
                    warm_start_x=None, warm_start_indices=None):
        """
        D3P dynamic-stride DDIM denoising.

        Optional warm-start for replan continuity (SDEdit-style):
            warm_start_x (B, H, A): pre-noised initial chunk to start denoising
                from instead of pure noise. Caller is responsible for noising
                via self.q_sample at the appropriate DDPM timestep.
            warm_start_indices (B,) long: per-env starting DDIM index. 0 = full
                noise (default behavior), max_ddim_steps = already clean.
        ...
        if warm_start_x is None:
            assert warm_start_indices is None, (
                "warm_start_x and warm_start_indices must be provided together"
            )
            x = torch.randn((B, self.horizon_steps, self.action_dim), device=device)
            current_indices = torch.zeros(B, dtype=torch.long, device=device)
        else:
            assert warm_start_indices is not None
            assert warm_start_x.shape == (B, self.horizon_steps, self.action_dim)
            assert warm_start_indices.shape == (B,)
            x = warm_start_x.to(device).float()
            current_indices = warm_start_indices.to(device).long().clamp(
                min=0, max=max_ddim_steps
            )
        ...
        padded_indices = torch.zeros((B, max_ft_steps), dtype=torch.long, device=device)
        valid_mask = torch.zeros((B, max_ft_steps), device=device)

        # Envs already at / past max_ddim_steps are immediately done.
        done = current_indices >= max_ddim_steps
